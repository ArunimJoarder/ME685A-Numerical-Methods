...
mat GS(mat A, mat b, double e1 = 0.01, double e2 = 0.001, double al = 1.9, int iter_max = 100){
    ...
    */
    ...
    while(ea1 > e1 || ea2 > e2 && counter < iter_max){
        ...
        for(int i = 0; i < n; i++){
            ...
            double a = (b[i][0] - sum + A[i][i]*x[i][0])/A[i][i];
            x_new.set(i,0,a);
            x.set(i,0,al*x_new[i][0] + (1-al)*x_prev[i][0]);
        }
...
double CN(mat B){
    ...
    for(int i = 0; i < A.size()[0]; i++){
        double max_n = 0;
        for(int j = 0; j < A.size()[1]; j++){
            max_n = max(fabs(A[i][j]), max_n);
        }
        A.setRow(i, A.row(i)/max_n);
    }
    // A.disp();
    // find inverse of normalised matrix
    mat A_i = inv(A);
    // (A_i*A).disp();
    cout<<row_sum_norm(A_i)<<endl;
    // return product of norms of both matrices
    return row_sum_norm(A)*row_sum_norm(A_i);
}

double dot(mat a, mat b){
    double dot = 0;
    // cout << a.size()[0] << " " << a.size()[1] << endl;
    if(a.size()[0] == 1 && b.size()[0] == 1 && a.size()[1] == b.size()[1]){
        for(int i = 0; i < a.size()[1]; i++){
            dot += a[0][i]*b[0][i];
        }
    }
    else if(a.size()[1] == 1 && b.size()[1] == 1 && a.size()[0] == b.size()[0]){
        for(int i = 0; i < a.size()[0]; i++){
            dot += a[i][0]*b[i][0];
        }
    }
...
pair<mat, mat> GramSchimdt(mat A, mat b){
    if(A.size()[0] != A.size()[1]){
        return make_pair(mat(),mat());
    }
    for(int i = 0; i < A.size()[0]; i++){
        A.setRow(i, A.row(i)/norm(A.row(i)));
        for(int j = 0; j < i; j++){
            mat rj = A.row(j);
            double nm1 = norm(rj);
            // rj.disp();
            ...
            mat ri = A.row(i);
            double dp = dot(vj, ri);
            double nm2 = norm(ri - dp*vj);
            A.setRow(i,(ri - dp*vj)/nm2);
            b.set(i,0, (b[i][0] - b[j][0]*dp/nm1)/nm2);
        }
    }
